...
var (
	...
	userColumnsWithoutDefault = []string{"accounts_id"}
	userColumnsWithDefault    = []string{"id", "email", "first_name", "last_name", "disabled", "properties", "created_at", "updated_at", "removed_at"}
...
func (o *User) Notes(mods ...qm.QueryMod) noteQuery {
	...
	query := Notes(queryMods...)
	queries.SetFrom(query.Query, "\"notes\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"notes\".*"})
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadNotes(e boil.Executor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`notes`),
		qm.WhereIn(`notes.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// Sets related.R.User appropriately.
func (o *User) AddNotes(exec boil.Executor, insert bool, related ...*Note) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
