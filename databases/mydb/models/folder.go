...
var FolderWhere = struct {
	...
	ID:        whereHelperint64{field: "\"folder\".\"id\""},
	Name:      whereHelpernull_String{field: "\"folder\".\"name\""},
	UserID:    whereHelperint64{field: "\"folder\".\"user_id\""},
	CreatedAt: whereHelpertime_Time{field: "\"folder\".\"created_at\""},
}
...
func (q folderQuery) One(exec boil.Executor) (*Folder, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models: failed to execute a one query for folder")
	}
...
func (q folderQuery) Count(exec boil.Executor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to count folder rows")
	}
...
func (q folderQuery) Exists(exec boil.Executor) (bool, error) {
	...
	err := q.Query.QueryRow(exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "models: failed to check if folder exists")
	}
...
func Folders(mods ...qm.QueryMod) folderQuery {
	mods = append(mods, qm.From("\"folder\""))
...
func FindFolder(exec boil.Executor, iD int64, selectCols ...string) (*Folder, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"folder\" where \"id\"=$1", sel,
	)
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models: unable to select from folder")
	}
...
func (o *Folder) Insert(exec boil.Executor, columns boil.Columns) error {
	if o == nil {
		return errors.New("models: no folder provided for insertion")
	}
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"folder\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"folder\" %sDEFAULT VALUES%s"
		}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to insert into folder")
	}
...
func (o *Folder) Update(exec boil.Executor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("models: unable to update folder, could not build whitelist")
		}

		cache.query = fmt.Sprintf("UPDATE \"folder\" SET %s WHERE %s",
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update folder row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by update for folder")
	}
...
func (q folderQuery) UpdateAll(exec boil.Executor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update all for folder")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to retrieve rows affected for folder")
	}
...
func (o FolderSlice) UpdateAll(exec boil.Executor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"folder\" SET %s WHERE %s",
...
func (o *Folder) Upsert(exec boil.Executor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("models: no folder provided for upsert")
	}
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("models: unable to upsert folder, could not build update column list")
		}
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"folder\"", updateOnConflict, ret, update, conflict, insert)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert folder")
	}
...
func (o *Folder) Delete(exec boil.Executor) (int64, error) {
	...
	result, err := exec.Exec(sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete from folder")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by delete for folder")
	}
...
func (q folderQuery) DeleteAll(exec boil.Executor) (int64, error) {
	...
	result, err := q.Query.Exec(exec)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete all from folder")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by deleteall for folder")
	}
...
func (o *FolderSlice) ReloadAll(exec boil.Executor) error {
	...
	sql := "SELECT \"folder\".* FROM \"folder\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, folderPrimaryKeyColumns, len(*o))
...
func FolderExists(exec boil.Executor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"folder\" where \"id\"=$1 limit 1)"
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "models: unable to check if folder exists")
	}
